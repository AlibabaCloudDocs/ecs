...
**Note:** You can use the parameter store of Operation Orchestration Service \(OOS\) in Cloud Assistant commands. For more information, see [t2069404.md\#]() |
...
By default, Cloud Assistant commands are run by the root user on Linux instances and by the system user on Windows instances. |
    |Password Name|The password of the execution user that is hosted in Operation Orchestration Service \(OOS\). This parameter is required only when the command is run by a regular user other than the system user on Windows instances. If the password of the execution user is not hosted in OOS, you must create and host the password in the parameter store of OOS. For more information, see [Create encryption parameters]() and [Run Cloud Assistant commands on Windows instances as a regular user](/intl.en-US/Deployment & Maintenance/Cloud assistant/DevOps practice/Run Cloud Assistant commands as a regular user.md). |
