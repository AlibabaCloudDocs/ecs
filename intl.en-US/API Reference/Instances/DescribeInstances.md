...
You can call this operation to query the details of one or more instances.

## Description

-   You can specify multiple request parameters to be queried. Specified parameters have logical AND relations. Only the specified parameters are included in the filter conditions. However, if InstanceIds is set to an empty JSON array, the filter conditions are considered to be valid and an empty response is returned.
-   If you are using a RAM user or a RAM role, an empty list is returned when the user or the role has no permissions to call this operation. You can include the `DryRun` parameter in your request to check whether the empty list is caused by the lack of permissions.
...
|InstanceIds|String|No|\["i-bp67acfmxazb4p\*\*\*\*", "i-bp67acfmxazb4p\*\*\*\*", ... "i-bp67acfmxazb4p\*\*\*\*"\]|The IDs of the instances. The value can be a JSON array that consists of up to 100 instance IDs. Separate multiple instance IDs with commas \(,\). |
...
|PublicIpAddresses|String|No|\["42.1.1.\*\*", "42.1.2.\*\*", … "42.1.10.\*\*"\]|The public IP addresses of the instances. The value can be a JSON array that consists of up to 100 IP addresses. Separate multiple IP addresses with commas \(,\). |
...
|InstanceChargeType|String|No|PostPaid|The billing methods of the instance. Valid values:
...
|CpuOptions|Struct| |The configuration details of the CPU. |
|CoreCount|Integer|2|The number of CPU cores. |
|Numa|String|2|The number of threads allocated. The value is set to 2. |
...
|InternetChargeType|String|PayByTraffic|The billing method for network usage. Valid values:
 ...
 -   PayByBandwidth
-   PayByTraffic |
|InternetMaxBandwidthIn|Integer|50|The maximum public inbound bandwidth. Unit: Mbit/s. |
|InternetMaxBandwidthOut|Integer|5|The maximum public outbound bandwidth. Unit: Mbit/s. |
...
|NetworkInterface| | | |
|Ipv6Sets|Array of Ipv6Set| |The IPv6 addresses assigned to the ENI. This parameter value is returned only when `AdditionalAttributes.N` is set to `NETWORK_PRIMARY_ENI_IP`. |
|Ipv6Set| | | |
...
|OSType|String|linux|The operating system type of the instance, which contains Windows Server and Linux. Valid values:
...
|LockReason|String|Recycling|The type of the reason why the instance was locked. Possible values:

 -   financial: The instance was locked due to overdue payments.
-   security: The instance was locked due to security reasons.
-   recycling: The preemptible instance was locked and pending for release.
...
|SpotDuration|Integer|1|The protection period of the preemptible instance. Unit: hours. Valid values: 0 to 6.
 ...
 -   The values 2, 3, 4, 5, and 6 of this parameter are in invitational preview. Submit a ticket if you want to use these values.
...
|StoppedMode|String|KeepCharging|Indicates whether the instance continues to be billed after it is stopped. Valid values:
 ...
 -   KeepCharging: The billing of the instance continues after the instance is stopped, and resources are reserved for the instance.
-   StopCharging: The billing of the instance stops after the instance is stopped. After the instance is stopped, its resources such as vCPUs, memory, and public IP address are released. You may be unable to restart the instance if some types of resources are out of stock in the current region.
...
|TotalCount|Integer|1|The total number of instances queried. |
