...
**Note:** When the `Ipv6Address.N` parameter is specified, the `Amount` parameter must be set to 1 and the `Ipv6AddressCount` parameter must be empty. |
...
|400|InvalidSpotPriceLimit.LowerThanPublicPrice|The specified parameter "spotPriceLimit" can't be lower than current public price.|The error message returned because the specified value of the SpotPriceLimit parameter is less than the current market price.|
...
|403|InvalidVSwitch.Ipv6NotTurnOn|%s|The error message returned because IPv6 is not enabled on your current vSwitch. Enable this feature and try again.|
