...
-   **Preparations**:
    ...
    -   Instance type selection: Call the [DescribeInstanceTypes](~~25620~~) operation to query the performance data of the instance type to which the instances to be created belong. You can also see [Best practices for instance type selection](~~58291~~) to learn about how to select instance types.
...
|ImageId|String|No|aliyun\_2\_1903\_x64\_20G\_alibase\_20200324.vhd|The ID of the image used to create the instances. You can call the [DescribeImages](~~25534~~) operation to query the available images. If you do not use the `LaunchTemplateId` or `LaunchTemplateName` parameter to specify an instance launch template, or do not use the `ImageFamily` parameter to select the latest available custom images, you must specify the `ImageId` parameter. |
...
-   If you do not set the `ImageId` parameter, and do not use the `ImageId` parameter in the launch template specified by the `LaunchTemplateId` or `LaunchTemplateName` parameter, you can set the ImageFamily parameter.
-   If you do not set the `ImageId` parameter, or do not use the `LaunchTemplateId` or `LaunchTemplateName` parameter, you can set the ImageFamily parameter. |
...
-   Select instance types: See [Instance families](~~25378~~) or call the [DescribeInstanceTypes](~~25620~~) operation to query the performance data of the instance type to which the instances to be created belong, or see [Select instance types](~~58291~~) to learn how to select instance types.
...
**Note:** When you create multiple ECS instances, you can set the `UniqueSuffix` parameter to specify different hostnames for these instances. You can also use the `name_prefix[begin_number,bits]name_suffix` format to name sequential hostnames. For example, if you set `HostName` to `k8s-node-[1,4]-ecshost`, the hostname of the first ECS instance is `k8s-node-0001-ecshost`. For more information, see [API FAQ](~~122617#howToAddSequentialSuffix~~).
...
**Note:** When the **pay-by-traffic** billing method is used, the peak inbound and outbound bandwidths are both the upper limits of bandwidths and used only for reference. In scenarios where demands exceed resource supplies, the maximum bandwidth may be limited. If you want guaranteed bandwidths for your instances, use the **pay-by-bandwidth** billing method. |
...
|SystemDisk.PerformanceLevel|String|No|PL1|The performance level of the ESSD used as the system disk. Valid values:
...
Default value: false. |
|DataDisk.N.KMSKeyId|String|No|0e478b7a-4262-4802-b8cb-00d3fb40\*\*\*\*|The ID of the KMS key used by data disk N. |
...
**Note:** You can attach only one secondary ENI when you create an ECS instance. After the instance is created, you can call the [CreateNetworkInterface](~~58504~~) and [AttachNetworkInterface](~~58515~~) operations to attach more secondary ENIs.
...
|RamRoleName|String|No|RAM\_Name|The name of the RAM role to bind. You can call the [ListRoles](~~28713~~) operation provided by RAM to query the instance RAM roles that you have created. |
...
|Tag.N.Value|String|No|TestValue|The value of tag N to be bound to the instance, disks, and the primary ENI. Valid values of N: 1 to 20. It can be an empty string. It can be up to 128 characters in length and cannot start with acs:. It cannot contain http:// or https://. |
...
-   false: The request is checked, and the instance is created if the check succeeds. Default value: false. |
...
|LaunchTemplateName|String|No|LaunchTemplate\_Name|The name of the instance launch template.
...
|PrivateIpAddress|String|No|10.1.2.1|The private IP address to be assigned to the instance.
...
**Note:** When the `Ipv6Address.N` parameter is specified, the `Amount` parameter must be set to 1 and the `Ipv6AddressCount` parameter must be specified. |
...
-   Open: The system automatically matches a private pool of the Open type for the instance to start up. If no matching private pools exist, the instance takes up capacity of public pool resources. If the parameter is set to Open, the `PrivatePoolOptions.Id` parameter can be empty.
...
|403|InstanceType.Offline|%s|The error message returned because the instance type is retired or insufficient.|
...
|403|SecurityRisk.3DVerification|We have detected a security risk with your default credit or debit card. Please proceed with verification via the link in your email.|The error message returned because we have detected security risks to your default credit card or debit card. Click the link in the email for verification.|
