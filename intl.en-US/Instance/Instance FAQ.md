...
-   FAQ about enterprise-level instances
    ...
    -   [Can I change a shared instance to an enterprise-level instance?](#section_90e_qq9_gmf)
-   FAQ about persistent memory optimized instances
    ...
    -   [I already have a Redis cluster based on regular memory and want to migrate business from this cluster to persistent memory optimized instances. How do I do? What items do I need to take note of?](#section_nrq_5r7_5mj)
...
-   FAQ about GPU-accelerated instances
    ...
    -   [Why is Windows Graphics unable to support DirectX-based features?](#section_5mx_en9_guy)
...
-   FAQ about preemptible instances
    ...
    -   [What is the relationship between the user-defined maximum hourly price and the current market price of a preemptible instance?](#section_67i_s25_aqc)
    -   [Am I charged the same price for all preemptible instances started at the same time?](#section_y3x_sk2_stu)
    -   [For the first hour after a preemptible instance is created, am I charged at a price that changes with the market price?](#section_scn_g76_t1n)
    -   [Can I view the market price of an instance type when I purchase a preemptible instance?](#section_jxg_3ko_6xo)
    ...
    -   FAQ about preemptible instances that do not have protection periods
        -   [Which is more cost-effective: a preemptible instance that has a protection period or a preemptible instance that does not have a protection period?](#section_8ph_ned_xbq)
        -   [Is the release rate of preemptible instances that do not have protection periods higher than that of preemptible instances that have protection periods?](#section_su1_6fg_q9h)
        -   [Are preemptible instances that do not have protection periods prioritized for release over those that have protection periods?](#section_ut8_cn4_od6)
        -   [If I have created a preemptible instance that does not have a protection period, do subsequently created preemptible instances also not have protection periods?](#section_le8_fur_3cx)
        -   [Am I notified 5 minutes before a preemptible instance is released even if the instance does not have a protection period?](#section_sdq_k1m_i7w)
        -   [Are fewer resources provided for preemptible instances that have protection periods than for those that do not have protection periods?](#section_npx_jel_rj2)
        -   [Can I convert between preemptible instances that have and do not have protection periods?](#section_0c7_5a0_glg)
-   FAQ about reserved instances
    ...
    -   [Can a zonal reserved instance be changed to a regional one?](#section_wry_8lc_jj4)
    ...
    -   [Can reserved instances be used to offset the storage and network usage bills of pay-as-you-go instances?](#section_prg_o9w_0de)
    ...
    -   [After I modify, split, or merge reserved instances, when does the operation take effect?](#section_kfw_q23_shl)
    ...
    -   [Can I use reserved instances to offset the bills of images of pay-as-you-go Windows instances?](#section_0w6_6mr_z27)
    -   [Can reserved instances be used to offset the bills of images of pay-as-you-go Linux instances?](#section_wm8_tf8_rgx)
...
-   FAQ about connecting to instances
    -   FAQ about Virtual Network Computing \(VNC\) management terminals
        -   [Does a VNC management terminal allow multiple users to simultaneously log on?](#section_mdm_w4q_wgb)
        ...
        -   [What do I do if a black screen appears after I am connected to a VNC management terminal?](#section_xrm_6sm_10m)
        ...
        -   [When I use Firefox to access a VNC management terminal, an error message appears, which indicates that the secure connection has failed. What do I do?](#section_l6t_kik_vmy)
...
-   FAQ about managing instances
    ...
    -   [Why is an ECS instance that has release protection enabled automatically released from a scaling group?](#section_so8_gdz_2i5)
...
-   FAQ about instance billing
    -   [Am I still charged for a pay-as-you-go instance after it is stopped manually or due to an overdue payment?](#section_bll_9fl_9qs)
    ...
    -   [Does the billing method for network usage of an instance change when I change the billing method of the instance from pay-as-you-go to subscription?](#section_4lm_u5o_2my)
...
If the resources of a specific instance type are sold out when you attempt to purchase an instance of that instance type within a specific region or zone, perform one of the following operations:

-   Select another region.
-   Select another zone.
-   Select another instance type.

If the resources that you want to purchase are still unavailable after you perform all of the preceding operations, try again later. Instance resources are dynamic. When resources are insufficient, Alibaba Cloud replenishes them as soon as possible.
...
It takes 1 or 2 minutes to create an ECS instance. After the instance is created, you can perform the following operations:

-   For an ECS instance that runs a Linux operating system, you can connect to the instance without the need to make other configurations. For more information, see [Connect to an ECS instance]().
...
If resources within the specified zone are insufficient to create an instance of your selected instance type, the instance fails to be created. A refund is made to your account by the system. If you do not receive a refund within half an hour, [submit a ticket](https://workorder-intl.console.aliyun.com/console.htm).
...
Shared instances belong to a series of instance families that are targeted at individuals or small and medium-sized websites. Compared with enterprise-level instances that each exclusively have their own resources, shared instances share resources. Therefore, shared instances do not provide consistent computing performance, but can help you reduce costs.

## What are the differences between enterprise-level and shared instances?

Enterprise-level instances use a CPU-bound scheduling scheme. Each vCPU is bound to a CPU hyperthread. Instances do not compete for CPU resources and provide consistent computing performance as guaranteed in the service-level agreement \(SLA\).
...
For information about business scenarios to which different enterprise-level instances are applicable, see [Instance families](/intl.en-US/Instance/Instance families.md) and [Best practices for instance type selection](/intl.en-US/Best Practices/Best practices for instance type selection.md).

## How is the network performance of enterprise-level instances?

The network performance of enterprise-level instances depends on their specifications. The higher the specifications of enterprise-level instances are, the higher network performance the instances can deliver. For more information about the network performance of different instance specifications, see [Instance families](/intl.en-US/Instance/Instance families.md).

## Which disk categories do enterprise-level instances support?

For information about the disk categories that enterprise-level instances support, see [Disk categories](/intl.en-US/Block Storage/Block Storage overview/Cloud disks.md).

## Which image types do enterprise-level instances support?

For information about the public images that enterprise-level instances support, see [Overview](/intl.en-US/Images/Public image/Overview.md).
...
For information about the limits on changing the instance types of enterprise-level instances, see [Instance families that support instance type changes](/intl.en-US/Instance/Change configurations/Instance families that support instance type changes.md).

## Can I change a shared instance to an enterprise-level instance?

Yes, you can change a shared instance to an enterprise-level instance. For more information, see [Instance families that support instance type changes](/intl.en-US/Instance/Change configurations/Instance families that support instance type changes.md).

## What characteristics do persistent memory optimized instances have?

Persistent memory optimized instances use high-capacity persistent memory and provide slower access and higher data durability than instances that use regular memory do. When persistent memory optimized instances are stopped or restarted, data in their persistent memory is not lost. Persistent memory can be used in the following ways:

-   Memory: You can move some data from regular memory to persistent memory, such as non-hot data that does not require high-speed storage access. Persistent memory offers large capacity at a lower price per GiB and can help reduce the total cost of ownership \(TCO\) per GiB of memory.
-   Local SSDs: Persistent memory can be used as ultra-high performance SSDs to offer a read/write latency as low as 400 nanoseconds. You can use persistent memory for core application databases that require consistent response time. You can also replace cache disks with persistent memory to obtain higher input/output operations per second \(IOPS\), higher bandwidth, and lower latency, and improve the business performance of the entire cluster.
...
**Note:** When you purchase persistent memory optimized instances, choose instance types named in the ecs.re6p-redis.<nx\>large format.

## I already have a Redis cluster based on regular memory and want to migrate business from this cluster to persistent memory optimized instances. How do I do? What items do I need to take note of?

Business consistency and data reliability must be ensured while you are migrating business from the Redis cluster to persistent memory optimized instances. We recommend that you first purchase a single persistent memory optimized instance and test it with a small amount of required business to check whether the basic performance and capacity model of the instance meet your business requirements. If the basic performance and capacity model of the instance meet your business requirements, you can purchase more persistent memory optimized instances and migrate your business from the Redis cluster.
...
To optimize performance or costs for I/O-intensive applications, you can select persistent memory optimized instances. You can use persistent memory optimized instances to solve common problems. Examples:

-   The latency of single SQL queries is high, or SQL queries require more consistent response time.
-   It takes an extended period of time to load resources on frontend servers of games, heavily loaded databases, and heavily loaded web servers.
-   You purchased large cloud disks or local disks for their high input/output operations per second \(IOPS\) and bandwidth but now have unused capacity and unnecessary costs.

You can use persistent memory optimized instances in typical I/O-intensive scenarios. Examples:
...
Cause: The mismatch between the kernel and kernel-devel package versions caused a driver compilation error when the driver was installed from the .rpm file.
...
## Why is Windows Graphics unable to support DirectX-based features?
...
ECS bare metal instances support enhanced SSDs \(ESSDs\), standard SSDs, and ultra disks. Up to 16 data disks can be attached to a single ECS bare metal instance.

## Can ECS bare metal instances have their configurations upgraded or downgraded? Do they support failover?

ECS bare metal instances cannot have their configurations upgraded or downgraded. ECS bare metal instances support failover. When the physical machine that hosts an ECS bare metal instance fails, the system fails the instance over to another physical machine. Data is retained in the data disks of the instance.

## How do I create an SCC instance?

You can create an SCC instance by using one of the following methods:

-   If you want to use only the remote direct memory access \(RDMA\) feature, log on to the ECS console. Create an SCC instance as described in [Create an SCC instance](/intl.en-US/Instance/Instance type families/Super Computing Cluster instance type family/Create an SCC instance.md).
-   If you want to use the HPC scheduler and the cluster resizing service in addition to RDMA, log on to the [E-HPC console](https://ehpc.console.aliyun.com). Create an SCC cluster and then create an SCC instance.
...
If you select an image that is customized for SCC and supports RDMA RoCE drivers and OFED stacks to create an SCC instance, you can use SCC RDMA by using the IB verbs API or implement RDMA-based communication by using the MPI.
...
After the one-hour protection period of a preemptible instance expires, the preemptible instance is automatically released if the current market price of the preemptible instance exceeds the user-defined maximum hourly price or if resources are insufficient.

## Am I notified when my preemptible instances are about to be released? How am I notified?

Yes, you are notified when your preemptible instances are about to be released. When your preemptible instance needs to be released due to a market price change or insufficient resources, the instance first enters the To Be Released state and then is automatically released in 5 minutes.
...
To check whether an instance is in the To Be Released state, you can view the [instance metadata](/intl.en-US/Instance/Manage instances/Manage instance metadata/Overview of ECS instance metadata.md) or call the [DescribeInstances](/intl.en-US/API Reference/Instances/DescribeInstances.md) operation to view the returned OperationLocks data.

## Can the data of a preemptible instance be automatically retained when the instance is released?

No, the data of a preemptible instance cannot be automatically retained when the instance is released. When a preemptible instance is no longer needed, we recommend that you back up your data and environment by following the instructions in [Create a snapshot](/intl.en-US/Snapshots/Use snapshots/Create a normal snapshot.md) and then release the instance. You can purchase preemptible instances at any time.
...
To create a preemptible instance, you can specify a maximum hourly price to bid for an instance type. If the current market price does not exceed your specified maximum hourly price, the preemptible instance that you request is created, and you are billed based on the current market price. For more information, see [Create a preemptible instance](/intl.en-US/Instance/Instance purchasing options/Preemptible instances/Create a preemptible instance.md).

## What is the relationship between the user-defined maximum hourly price and the current market price of a preemptible instance?

If the current market price does not exceed the user-defined maximum hourly price, the preemptible instance is created, and you are billed based on the current market price. After a preemptible instance is created, the instance enters a protection period in which it cannot be automatically released due to insufficient resources or changes to the market price.

After the protection period expires, the system checks the current market price and resource availability of the instance type every 5 minutes. If the current market price exceeds the user-defined maximum hourly price or if resources of the instance type are insufficient, the preemptible instance that is in the Running state is released.
...
## For the first hour after a preemptible instance is created, am I charged at a price that changes with the market price?
...
## Can I view the market price of an instance type when I purchase a preemptible instance?

Yes, when you create a preemptible instance in the ECS console, you can view the market price and historical prices of each selected instance type. The total instance price is displayed in the lower part of the instance buy page. The total instance price includes the instance type, storage, and bandwidth fees. The instance type fee is the current market price of the selected instance type.
...
Yes, you continue to be billed for preemptible instances after they are stopped. When a preemptible instance is no longer needed, we recommend that you back up your data and environment by following the instructions in [Create a snapshot](/intl.en-US/Snapshots/Use snapshots/Create a normal snapshot.md) and then release the instance. You can purchase preemptible instances at any time.
...
vCPU-based quotas apply to preemptible instances within each account. Instance-based quotas do not apply. When you create a preemptible instance, you can view the available vCPU-based quota after you select an instance type. For more information, see the "Instance limits" section in [Limits](/intl.en-US/Product Introduction/Limits.mdsection_tbg_zdx_wdb).
...
Whether the Preemptible Instance option is available depends on your ECS resource usage.

## Which is more cost-effective: a preemptible instance that has a protection period or a preemptible instance that does not have a protection period?

Preemptible instances that do not have protection periods are more cost-effective and have a discount of 10% compared with preemptible instances that have protection periods.

## Is the release rate of preemptible instances that do not have protection periods higher than that of preemptible instances that have protection periods?

You can check the release rate of each instance type on the instance buy page in the ECS console regardless of whether you attempt to create preemptible instances that have or do not have protection periods. For each instance type, the release rate is determined by the bidding policy and the supply and demand relationships of resources.

![release-rate](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/en-US/6554873061/p170286.png)

## Are preemptible instances that do not have protection periods prioritized for release over those that have protection periods?

No, preemptible instances that do not have protection periods are not prioritized for release over those that have protection periods.

## If I have created a preemptible instance that does not have a protection period, do subsequently created preemptible instances also not have protection periods?

No, preemptible instances that do not have protection periods are created only if you choose not to configure a protection period for them. By default, each preemptible instance is created with a protection period of 1 hour.
...
## Am I notified 5 minutes before a preemptible instance is released even if the instance does not have a protection period?

Yes, you are notified 5 minutes before a preemptible instance is released even if the instance does not have a protection period.

## Are fewer resources provided for preemptible instances that have protection periods than for those that do not have protection periods?

No, the same amount of resources are provided for preemptible instances that have and do not have protection periods.

## Can I convert between preemptible instances that have and do not have protection periods?

No, you cannot convert between preemptible instances that have and do not have protection periods. By default, each created preemptible instance has a protection period of one hour. The protection periods of preemptible instances can be configured only when the instances are created by calling API operations. The protection period settings of preemptible instances cannot be changed after the instances are created.

## What are reserved instances?

Reserved instances automatically match pay-as-you-go instances \(excluding preemptible instances\) within your account to offset the bills of the pay-as-you-go instances. Reserved instances can also be used to reserve instance resources. A combination of reserved instances and pay-as-you-go instances provides higher cost-effectiveness and a higher degree of flexibility compared with subscription instances.
...
Reserved instances support both Windows and Linux operating systems. For example, a reserved Linux instance can be applied to matched pay-as-you-go Linux instances regardless of the image type such as public images, custom images, shared images, or Alibaba Cloud Marketplace images.
...
One regional ecs.c5.xlarge reserved instance in the China \(Qingdao\) region. This reserved instance is named C5RI.

After you purchase the C5RI reserved instance, it matches the C5PAYG-b pay-as-you-go instance to offset the bills of the C5PAYG-b instance.

You release C5PAYG-b and start another Linux instance of the same instance type named C5PAYG-c in Qingdao Zone C. The C5RI reserved instance continues to match C5PAYG-c.
...
You have one regional ecs.g5.4xlarge reserved instance. It can be applied to offset the bills of one ecs.g5.4xlarge pay-as-you-go instance, two ecs.g5.2xlarge pay-as-you-go instances, or four ecs.g5.xlarge pay-as-you-go instances.
...
No, zonal reserved instances do not provide instance size flexibility. A zonal reserved instance can be applied only to pay-as-you-go instances of the same instance type as that of the zonal reserved instance.

## Do zonal reserved instances provide zone flexibility?

No, zonal reserved instances do not provide zone flexibility. A zonal reserved instance can be applied only to pay-as-you-go instances in the same zone where the zonal reserved instance resides.

## Can a zonal reserved instance be changed to a regional one?

Yes, a zonal reserved instance can be changed to a regional one. You can change the scope of a purchased reserved instance in the following ways:
...
No, the scope of a reserved instance cannot be changed from one region to another. For example, if you have a zonal reserved instance in Hangzhou Zone B, you can change the instance scope to another zone within the China \(Hangzhou\) region or change the instance to a regional reserved instance in the China \(Hangzhou\) region. However, you cannot change the scope of the zonal reserved instance to a zone of another region, or change the instance to a regional reserved instance in another region.

## Can reserved instances be used across accounts?

No, reserved instances cannot be used across accounts.

## Can reserved instances be used to offset the storage and network usage bills of pay-as-you-go instances?

No, reserved instances cannot be used to offset the storage or network usage bills of pay-as-you-go instances. Reserved instances are used to offset the bills of computing resources of pay-as-you-go instances, including vCPUs and memory. For pay-as-you-go Windows instances, reserved instances can also be used to offset the bills of images.
...
You are separately billed for reserved instances and can choose the All Upfront, Partial Upfront, or No Upfront payment option.

The term of a reserved instance starts from when the instance is purchased. You are charged based on the payment option that you selected, regardless of whether the reserved instance matches pay-as-you-go instances. The All Upfront payment option is most cost-effective.

## When does a reserved instance take effect after it is purchased?

A reserved instance takes effect and you are billed starting on the hour when the reserved instance is purchased. The reserved instance becomes invalid at 00:00:00 on the day after the reserved instance expires. For example, if you purchase a reserved instance with a term of one year at 13:45:00 on February 26, 2019, the reserved instance takes effect and you are billed starting from 13:00:00 on February 26, 2019. The reserved instance becomes invalid at 00:00:00 on February 27, 2020. If you already have eligible pay-as-you-go instances when you purchase a reserved instance, the reserved instance is applied to offset the bills generated by the pay-as-you-go instances starting from the hour of 13:00 to 14:00 on February 26, 2019 until the reserved instance becomes invalid.

## After I modify, split, or merge reserved instances, when does the operation take effect?

When reserved instances are modified, split, or merged, new reserved instances are generated, and the original ones become invalid. The new reserved instances take effect and the original ones become invalid on the hour when the modify, split, or merge operation is performed. For example, you split one ecs.g5.2xlarge zonal reserved instance RI1 into two zonal ecs.g5.xlarge reserved instances RI2 and RI3 at 13:45:00 on February 26, 2019. At 13:00:00 on February 26, 2019, RI1 becomes invalid, and RI2 and RI3 take effect. Starting from 13:00:00 on February 26, 2019, the instance type eligible for resource reservation and billing discounts is ecs.g5.xlarge, not ecs.g5.2xlarge any more. If RI2 and RI3 immediately match pay-as-you-go instances after they take effect, RI2 and RI3 are also applied to offset the hourly bills of ecs.g5.xlarge pay-as-you-go instances starting from 13:00:00 on February 26, 2019.
...
## Can I use reserved instances to offset the bills of images of pay-as-you-go Windows instances?

Yes, reserved instances can be used to offset the bills of images of pay-as-you-go Windows instances. This is because the Windows reserved instance fees already include Windows image fees.

## Can reserved instances be used to offset the bills of images of pay-as-you-go Linux instances?

No, reserved instances cannot be used to offset the bills of images of pay-as-you-go Linux instances.
...
Yes, a reserved instance can be applied to more than one pay-as-you-go instance at the same time. Reserved instances check for eligible pay-as-you-go bills on an hourly basis and offset bills based on the computing power of the reserved instances.

**Note:** The computing power and term of each reserved instance are fixed. You cannot increase the computing power of a reserved instance by shortening the term of the reserved instance.

For example, you have a reserved instance that has the following attributes:

-   Instance type: c5.large
-   Instance quantity: 1, which indicates that the reserved instance can match one pay-as-you-go instance of the specified instance type
-   Term: one year

The following examples demonstrate how the reserved instance is applied based on the running status of pay-as-you-go instances:

-   Each of the six c5.large pay-as-you-go instances runs for 1 hour. Each of these pay-as-you-go instances consumes 1 hour of computing power equal to that delivered by the c5.large reserved instance every hour. The reserved instance is randomly applied to one of the pay-as-you-go instances. You cannot configure the reserved instance to be applied to all the six pay-as-you-go instances by shortening the term of the reserved instance to two months.
-   Each of the six c5.large pay-as-you-go instances runs for 10 minutes. The six instances consume 10 minutes of computing power each and in total consume the amount of computing power that the c5.large reserved instance can deliver every hour. The reserved instance is applied to all the six pay-as-you-go instances.
-   Each of the six c5.large pay-as-you-go instances runs for 15 minutes. The six instances consume 15 minutes of computing power each and in total consume the amount of computing power that exceeds the c5.large reserved instance can deliver every hour. The reserved instance is randomly applied to the pay-as-you-go instances to offset the bills for 1 hour of computing power.

## Does a VNC management terminal allow multiple users to simultaneously log on?

No, a VNC management terminal allows only a single user to log on at a time.
...
Authentication fails if the entered password is incorrect. Perform the following troubleshooting operations:
...
## What do I do if a black screen appears after I am connected to a VNC management terminal?
...
-   For a Windows instance, choose **Send Remote Call** \> **CTRL+ALT+DELETE** in the upper-left corner to go to the logon interface.
...
VNC management terminals support only Internet Explorer 10 and later.

We recommend that you use Google Chrome that is more compatible with the Alibaba Cloud Management Console.

## When I use Firefox to access a VNC management terminal, an error message appears, which indicates that the secure connection has failed. What do I do?
...
Linux instances support SSH for remote connection. You can use one of the following methods to log on to a Linux instance:
...
-   Use the Remote Desktop Connection \(RDC\) tool on your computer.
    ...
    You cannot adjust the desktop resolution of a Windows instance after you connect to the instance from your computer by using RDC. You must adjust the display settings in RDC on your computer and then connect to the instance by using RDC to apply the new display settings to the instance.
...
If you deploy DPDK applications on ECS instances of the sixth-generation and later instance families such as g6, c6, and r6, the bound igb\_uio port may not be detected when you use Pktgen-DPDK to perform tests, and the following error is reported:
...
-   A maximum of 16 data disks can be attached to each ECS instance. You can resize a data disk but cannot reduce the size of a data disk after it is extended.

    **Note:** Typically, a maximum of 16 data disks can be attached to a single instance. For some instance families such as g6se, more than 16 data disks can be attached to a single instance. For more information, see the "Elastic Block Storage \(EBS\) limits" section in [Limits](/intl.en-US/Product Introduction/Limits.mdBlockStorageQuota).
...
1.  After the development is complete, you can generate an intermediate QAR file during the compilation stage and upload the file to an OSS bucket. You can upload this file only to an OSS bucket within the China \(Hangzhou\) region. Then, you can register the QAR file information with Alibaba Cloud by calling an API operation.

    We recommend that you use the free Intel Quartus development suite to complete development, compilation, and simulation operations in the cloud.
...
If you want to restore the f1 instance to its initial status, call an API operation to erase the burned FPGA image from the f1 instance.
...
4.  Click **Connect**.
5.  In the right-side panel, select the destination directory. In Windows, use the current working directory. In Linux, select the htdocs directory. In the left-side pane, select files and click the Upload icon to upload the files.

    **Note:** If you attempt to install Yummy FTP and are prompted with a message similar to "Your security preferences allow the installation of applications only from the Mac App Store and authorized developers", perform the following steps:
...
Problem description: The system does not respond when you select an option from the GRUB menu on system startup. After you mount the LiveCD image to the ECS instance, you can log on to the instance and confirm that the file system privileges are correct and that the message logs show no exceptions.
...
The servers provided by Alibaba Cloud ECS are not multimedia servers and do not provide sound or video card components. Sound or video cards cannot be added to ECS instances.
...
To replace the automatically assigned public IP address of an ECS instance with an EIP, make sure that the instance uses the pay-by-bandwidth billing method for network usage and that you have purchased an EIP. Then, use one of the following methods:
...
You can go to the Renewal page to view subscription ECS instances in all regions within your account.
...
If an instance cannot be stopped by using a regular shutdown procedure, you can forcibly stop the instance. A forced stop is equivalent to disconnecting the power from the instance, and can cause loss of unsaved data.
...
## Why is an ECS instance that has release protection enabled automatically released from a scaling group?
...
If the file or directory permissions are incorrectly configured on the web server, a 403 error is reported when you access a website hosted on the instance. Before you adjust a file or directory, you must identify the identity under which the file or directory process is running.
...
To change the owners and owner groups, run the chown command. For example, you can run the chown -R www.www /alidata/www/phpwind/ command to change the owners and owner groups of all files and directories in the /alidata/www/phpwind directory to the www account.
...
If the maximum number of pay-as-you-go instances that you can purchase has been reached, you cannot purchase more pay-as-you-go instances. For more information, see the "Instance limits" section in [Limits](/intl.en-US/Product Introduction/Limits.mdsection_tbg_zdx_wdb). You can log on to the ECS console and click Privileges in the upper-right corner of the Overview page to view your resource quotas. For more information, see [View quotas \(old version\)]().
...
## Am I still charged for a pay-as-you-go instance after it is stopped manually or due to an overdue payment?
...
If one of the preceding errors is reported, accordingly adjust the instance.
...
The billing method of your ECS instance is changed after the order is paid. It can take up to 4 seconds to change the billing method of 20 instances. After the billing method is changed, you can see that the billing method of your instance has been changed to **Subscription** in the ECS console.
...
## Does the billing method for network usage of an instance change when I change the billing method of the instance from pay-as-you-go to subscription?
...
If one of the preceding errors is reported, accordingly adjust the instance. If the problem persists, [submit a ticket](https://workorder-intl.console.aliyun.com/#/ticket/createIndex).
