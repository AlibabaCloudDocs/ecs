...
-   AccessKey相当于用户密码，AccessKey用于调用API，而用户密码用于登录ECS控制台。其中AccessKeyID是访问者身份，AccessKeySecret是加密签名字符串和服务器端验证签名字符串的密钥，必须严格保密谨防泄露。更多详情，请参见[创建AccessKey]()。
-   云服务器ECS提供了多种编程语言的SDK及第三方SDK，可以免去您签名的烦恼。更多详情，请下载[SDK](https://github.com/aliyun)。

## 步骤一：构造规范化请求字符串

1.  排序参数。排序规则以首字母顺序排序，排序参数包括[公共请求参数](/intl.zh-CN/API参考/HTTP调用方式/公共参数.md)和接口自定义参数，不包括公共请求参数中的`Signature`参数。
...
4.  使用与号（`&`）连接编码后的请求参数，注意参数排序与[步骤1](#section_czy_tyb_wdb)一致。

现在，您得到了规范化请求字符串（CanonicalizedQueryString），其结构遵循请求结构。详情请参见[请求结构](/intl.zh-CN/API参考/HTTP调用方式/请求结构.md)。
...
2.  按照[RFC2104](http://www.ietf.org/rfc/rfc2104.txt)的定义，计算待签名字符串`StringToSign`的HMAC-SHA1值。示例使用的是Java Base64编码方法。
    ...
    **说明：** 计算签名时，RFC2104规定的Key值是您的`AccessKeySecret`并加上与号（`&`\)，其ASCII值为38。更多详情，请参见[创建AccessKey]()。
...
4.  添加[RFC3986](http://tools.ietf.org/html/rfc3986)规则编码后的`Signature=OLeaidS1JvxuMvnyHOwuJ%2BuX5qY%3D`到上文[步骤1](#section_czy_tyb_wdb)的URL中。
...
依然以调用[DescribeRegions](/intl.zh-CN/API参考/地域/DescribeRegions.md)查询地域为例。假设您获得了`AccessKeyID=testid`以及`AccessKeySecret=testsecret`，并且假定所有请求参数放在一个Java `Map<String, String>`对象里。
...
返回结果列举了地域和地域ID列表。如果您在提交请求时，指定参数`Format=JSON`，那么返回结果的格式为JSON格式而不是XML格式。更多详情，请参见[返回结果](/intl.zh-CN/API参考/HTTP调用方式/返回结果.md)。
