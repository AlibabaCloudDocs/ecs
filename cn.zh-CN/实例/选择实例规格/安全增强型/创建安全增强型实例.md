...
在控制台创建安全增强型实例的步骤与创建普通实例的类似，但需要注意一些特定选项。本步骤以c6t为例，重点介绍该实例相关的特定配置，如果您想了解其他通用配置，请参见[使用向导创建实例](/cn.zh-CN/实例/创建实例/使用向导创建实例.md)。
...
4.  完成基础配置。
    ...
    -   **实例**：筛选并选中一款c6t规格。
    -   **镜像**：支持**Alibaba Cloud Linux 2.1903 64位 可信版**和**CentOS 7.8 64位 可信版**，您可以按需选择任一镜像。
...
|参数|说明|示例|
|--|--|--|
|InstanceType|指定安全增强型实例规格。目前安全增强型实例规格族包括：-   g6t
-   c6t
