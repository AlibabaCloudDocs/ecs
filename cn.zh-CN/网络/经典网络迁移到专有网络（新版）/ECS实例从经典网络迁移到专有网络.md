...
-   负载均衡SLB的虚拟服务器组（VServerGroup）中如果添加了该ECS实例，迁移后ECS实例不会自动挂载到SLB。关于如何重新添加虚拟服务器组，请参见[编辑虚拟服务器组](/cn.zh-CN/传统型负载均衡CLB/用户指南/后端服务器/虚拟服务器组/编辑虚拟服务器组.md)。 |
...
**说明：** 如果您已创建虚拟交换机，但下拉列表中找不到，表示您迁移后的实例和虚拟交换机不在同一个可用区，您需要在目标可用区重新创建虚拟交换机。具体操作，请参见[创建交换机](/cn.zh-CN/专有网络和交换机/管理交换机/创建交换机.md)。 |
        ...
        -   如果您选择改变内网IP的迁移方案，请提前配置好经典网络实例的ClassicLink链接失效与专有网络的联通。具体操作，请参见[经典网络和专有网络互通](/cn.zh-CN/网络/经典网络和专有网络互通.md)。 |
...
4.  检查内网环境和业务运行环境。
    ...
    |场景|迁移计划|后续操作|
    |--|----|----|
    |所有ECS统一迁移到同一个VPC|**选择迁移的目标专有网络**中选择**（默认）系统自动创建专有网络，网段：10.0.0.0/8**。|检查您的业务系统是否正常运行。|
...
2.  在其他云产品白名单中移除废弃的内网IP地址，例如，[RDS](/cn.zh-CN/云数据库 RDS 简介/什么是云数据库RDS.md)、[SLB](/cn.zh-CN/传统型负载均衡CLB/产品简介/什么是负载均衡.md)、[OSS](/cn.zh-CN/产品简介/什么是对象存储OSS.md)或者[容器服务](/cn.zh-CN/产品简介/什么是容器服务.md)等。
...
-   [云服务器ECS升级专有网络注意事项]()
-   [云数据库Redis切换为专有网络](/cn.zh-CN/用户指南/管理实例/管理网络连接/切换为专有网络.md)
